...
| PostgreSQL server and the `postgres` OS user | Project database creation and schema application | `scripts/install-switchyard-prereqs` installs the server package and then runs `scripts/ensure-postgres-cluster`, which verifies the admin socket `switchyard new` connects to. On Arch-family hosts the package leaves no cluster at all, so that helper runs `initdb` once -- only into a directory holding neither a cluster nor any other file -- and then `systemctl enable --now postgresql.service`. An existing cluster is started, never re-initialized. On Debian-family hosts the cluster stays the distro's: the helper starts the service and names `pg_createcluster` rather than building a second cluster. When it cannot finish, the installer stops with the copyable procedure instead of reporting a complete install, and `switchyard new`'s database preflight names the same remedy rather than only the raw `psql` socket error (SYRD-235). |
