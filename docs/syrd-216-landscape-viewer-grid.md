...
The fixture strips `TMUX` as well as setting `TMUX_TMPDIR`, and asserts its
socket is its own before creating or killing anything. A tmux command with
...
Mutation coverage: 15 mutants across the grid rule and its aspect predicate, the
span arithmetic, the checksum, the tree shape, the hook's event and its
installation in both builders, and the helper's use of what it measured.
