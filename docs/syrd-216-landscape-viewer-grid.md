...
The provenance check lives at the teardown, not only at the start, and that
distinction was itself a defect worth recording. The first version asked
`display-message` in `__init__`, before any server existed: tmux failed, the
empty answer was read as "nothing unexpected", and the guard passed for the one
reason it must never pass. `kill()` now resolves the running server and refuses
to issue `kill-server` unless that socket is inside its own directory, `verify()`
refuses an absent answer rather than treating it as reassurance, and the first
session of every case goes through `start()`, which proves what was just
created. Three cases test the refusals rather than the happy path: a fixture
with nothing running, one pointed at another server -- which must survive
untouched -- and one that cannot place its server at all.
...
Mutation coverage: 19 mutants across the grid rule and its aspect predicate, the
...
One survivor, and it is an equivalent mutant rather than a gap: dropping `-w`
from `set-hook` changes nothing, because a `session:window` target already
scopes the hook to the window. That was measured rather than assumed -- with and
