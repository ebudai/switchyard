...
Mutation coverage: 12 mutants across the grid rule, the span arithmetic, the
checksum, the tree shape and both call sites' pane counts. No survivors. Four
survived a first pass: the remainder-column convention was unasserted, the
single-row special case turned out to be dead weight and was removed rather than
tested, and both builders would have passed with their pane count hardcoded to
five until each was driven at six as well.
