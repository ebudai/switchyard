...
**Focused:** `operator_display_recovery_test` has 55 checks (13 new). Its
changes:
...
**Mutation: 7/7 killed.** The mutants:
...
The live UAT, repeated as the Director wrote it. After this release is
installed and mefp's tenant-control tooling is restaged:

1. Close only the Director's display attachment.
2. Run `switchyard recover-display mefp` from the desktop operator's shell,
   without setting any role variable.
3. Confirm the same Director worker comes back and is usable.
4. Confirm that cross-project recovery and ordinary non-Director presentation
   changes are refused.
5. Confirm the recovery is recorded as `operator:<desktop-user>` in the history.

The e2e case stands in for sudo's own setuid hop and for the tmux
reattachment itself. Only the live check covers those two.
