...
`switchyard paste-image <project> <role>`, in `scripts/image_handoff.py`, wired
into the launcher as an **unprivileged** command.

## Authorization

The desktop owner runs it, as themselves. There is no endpoint a role can call,
so nothing new is reachable from inside a pane — which is the point, because
every role on these hosts shares one Unix UID and anything reachable by one is
reachable by all of them.

Running it *as the tenant* is refused by identity, before the clipboard is
touched:
...
It is registered unprivileged on purpose. Escalating it would read root's
clipboard — there isn't one — and the authority being passed along is precisely
the owner's own.

## What may cross

* exactly one `image/*` type, chosen from the clipboard's advertised types
  (`wl-paste --list-types`, metadata only), PNG preferred;
* nothing else: a clipboard holding text is a refusal that names what was there,
  not a failure;
* refused above `MAX_IMAGE_BYTES` (10 MiB), before anything is written;
* the path is chosen by the helper. A caller never names one.

## Where it lands

`<role workdir>/.switchyard/paste/paste-<random>.<ext>`, mode `0640`, with one
ACL entry granting read to the tenant uid through `desktop_access.grant()` —
which already refuses any change that would broaden an unrelated masked entry.

Inside the role's own working directory because of Codex issue #46391: an
`@`-attachment silently degrades to plain text when the session and process
working directories differ. Under the role's own cwd that condition cannot
arise. The directory carries a `.gitignore` of `*`, so a handed-over image can
never become a commit.

## Delivery

The pane is proven live by reading which provider is actually running in it
(`live_cli_for_role`), not by trusting configuration — that is both the liveness
check and what selects the reference form:

* Claude — `Analyze this image: <path>`
* Codex — `@<path>`

both delivered through `directorctl send`, the same sender the notify listener
uses. A provider with no documented file-based image path is refused *before*
the clipboard is read.

## Cleanup and retry

Fails closed. A delivery that raises removes the image and says so, so a failed
handoff leaves nothing behind. Unclaimed images are swept past
`SPOOL_TTL_SECONDS` (120s) on the next use — cleanup runs on the same path that
creates files, rather than depending on a timer that might not run. Retry is
simply running the command again.

## Coverage

`tests/image_handoff_test.py`, 37 checks, one per property above: the tenant is
refused and the clipboard never read; text is refused naming what was there; PNG
is preferred; an oversized image is refused with nothing written; the file lands
under the role's own workdir at `0640` with its `.gitignore`; the grant is one
entry, read-only, on one file; each provider gets its own documented form, to its
own pane; a dead pane and an unsupported provider are both refused before
reading; an unknown role is named with the ones that exist; a failed delivery
leaves no picture; a stale image is swept and a fresh one is not; and the
operator's line names what crossed without any image bytes in it.

Mutation, 4 mutants over the safety decisions, all killed: dropping the
owner-identity refusal; dropping the size cap; accepting any MIME type as an
image; and not removing the file when delivery fails.

## What is still unproven

The live paste on Zorin. Everything above is driven through injected seams —
there is no compositor in this environment and no `test19` — so what is verified
is the boundary, not that GNOME hands the bytes over or that either provider
renders the attachment. The UAT sequence in the design section is what settles
that, and it is the User's to run.
