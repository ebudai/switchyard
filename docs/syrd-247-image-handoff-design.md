...
## What is still to verify before implementation

The conveyance is half settled and half open.

**Claude: settled, from primary documentation.** Claude Code's own docs list
three ways to add an image during an interactive session, and the third needs no
clipboard at all:

> 3. Provide an image path to Claude, for example "Analyze this image:
> /path/to/your/image.png"

That is ordinary text in the composer, which is exactly what `directorctl send`
already delivers. The handoff works for Claude as designed.

**Codex: open.** Checked against the installed CLI on this host:
...
Codex takes an image by path — but at **startup**, for the *initial* prompt. A
Switchyard role is a long-running session, so `-i` does not reach it, and whether
a path in the composer becomes an image attachment rather than a file read is
being verified against the binary rather than assumed.

So the design holds for Claude today. For Codex the conveyance is the one part
that may have to change, and the other five properties — owner-only
authorization, bounded validated data, live runtime identity, no widened grant,
fail-closed cleanup — do not depend on which conveyance is used.

If Codex cannot be handed an image mid-session by path, the remaining options are
narrower than they look, and each needs its own argument rather than a menu put
to the User:

* a per-handoff, per-role, short-lived clipboard the provider *can* read — which
  must not become a host-wide Xvfb or compositor dependency, and must not outlive
  the handoff;
* delivering the image at role start, where `-i` does work, which changes when a
  paste is possible rather than whether;
* a provider-side feature request, which is not a fix here.

That choice should be made on evidence from the binaries, not on preference, and
it is the one thing this design deliberately leaves open for review.
