...
## Final Sign-Off: bounded to the current visit

The Director found that `active_work_delivery` matched records by ticket, role
and stage but not by *visit*. A ticket sent to Ops, routed back to analysis
and returned to Ops reported the first visit's send as `delivered`, hiding a
pending or dead-lettered second notice. The same was true of the
pre-existing `active_work_notified_at`, which uses the same send lookup.

Both lookups are now bounded to `entered_current_state_at`: a send traced
at or after the current stage entry, and a queued notice created at or after
it. A row with no recorded entry (legacy) is left unbounded rather than
hidden.

**Measured, because the bound depends on it:** `entered_current_state_at`
records the ticket's update *before* the transition. The trigger reads
`NEW.updated_at` on the state-changing statement, before the route moves it.
In a real route, entry stayed at the seed's time while `updated_at` moved.
It is still a sound bound for this purpose. A second visit's entry is at or
after the route back, which is after the first visit's send, and a notice for
the new visit is enqueued after it. The lag itself is pre-existing and left
as it is; it is worth its own look.

`ticket_board_postgres_backend_test` dated its send traces `2026-07-10`,
before its tickets' seeded stage entry: a send that could not have happened.
Those six traces are now dated `clock_timestamp()`. Every assertion is
unchanged and passes.

## Retry (Director's decision)

No new redelivery authority for this ticket. MEFP-1 has no prior send, so its
live retry is the ordinary Director path once this release is deployed:
return it to analysis, then route it to Ops.

## Originally raised: an explicit retry
...
- **`tests/active_work_delivery_state_test.py`: 23 checks,** on a real
  ...
  and an ownerless ticket reporting nothing. It also covers the Director's
  two-visit case through the real `route_ticket`: the first visit is sent;
  after the route back and the return, the second visit is `pending` and then
  `failed`, never the first visit's `delivered`. And an earlier visit's dead
  letter does not count as this visit's failure. Every event is dated from
  the ticket's actual stage entry.
...
- **Mutation: 13/13 killed** across classifier, requeue, evidence, delivery
  state, card and the two visit bounds. The trace bound dies on the two-visit
  case; the queue bound dies on the earlier-visit dead letter.
