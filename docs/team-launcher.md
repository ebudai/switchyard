...
## Starting a tenant you do not own
...
unable to start their own project, and the two obvious fixes are worse than the
problem: a stored password is a reusable credential, and a broad sudo grant on
the owner account hands over every command the owner could ever run.

Provisioning records that human instead, and installs a narrow bridge for them.
...
secret, and nothing else is recorded -- there is no field in it anyone could
replay. No password is created, stored or retrieved anywhere in this path, and
role accounts stay locked for password login.

**What the bridge will do.** `switchyard-tenant-control <project> <operation>`
takes two arguments and nothing else -- no executable, no config path, no source
path, no shell fragment, no environment override, no role identity:

* **Who.** The caller is `SUDO_UID`, which sudo resolves from the kernel rather
  than copying from the environment, so a caller cannot present someone else's
...
* **Where.** The owner, the launcher and the project all come from the grant.
  ...
  authority.
* **Pinned, all the way down.** The launcher is the shared release entrypoint
  `/opt/switchyard/current/switchyard`, and *every* component on the way to it
  -- each directory, and each symlink, and each symlink's own chain -- must be
  owned by root and not group- or other-writable. Checking only the final file
  is not enough: the tenant's own deployed tree lives under the owner's home,
  where the owner can replace the `current` symlink or any directory above the
  launcher, so a root-owned JSON string and a root-owned leaf would still leave
  the owner choosing what the authorized human's `switchyard <project>` ran.
  The owner is not trusted here even though the program then runs as the owner:
  being pinned is what the grant promises the human who invoked it. The bridge
  execs the *resolved* path, so the program that runs is the one whose chain was
  just checked.
* **How.** The environment is rebuilt, not inherited -- a caller who chooses
...
separation the role UIDs exist to create.

**Repair and upgrade.** An installed grant wins over whoever is running the
repair, so `switchyard upgrade` and the role-account migration reinstall the
human already recorded rather than the operator at the keyboard, and a rerun
writes the same bytes. A tenant provisioned before the bridge existed gains one
from a repair. The controller is regenerated by root, never read back from
`plan.json` -- a tenant that could name its own controller could hand its owner
account to anyone.

**Teardown** removes the sudoers rule, the grant and the staged helper, so a
torn-down tenant leaves no part of its bridge behind.
