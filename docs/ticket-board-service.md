...
a designer or auditor exists, and role *names*. Anything that cannot be
regenerated and cannot be trusted is refused with the reason rather than guessed
at -- a database name that is not the generated one, for instance, because root
will not run a project's SQL against a database it did not choose.
