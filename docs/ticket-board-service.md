...
the hash of the line before it, so an entry removed, altered or reordered
without the hashes after it being recomputed breaks the chain, and
`switchyard rollout-log` says so.

**Where the trust boundary actually is.** Root ownership is the boundary, and it
is the one this exists for: the project account is every role, and a role cannot
edit or delete the evidence of a run it was the subject of. The chain is an
integrity check layered on that -- it catches accidental edits, truncated or
interrupted writes, and hand-deletions that do not recompute what follows. It is
**not** an authenticity proof against root. The entries and the hashes linking
them live in one root-owned file, so a root that rewrites it and recomputes the
chain leaves a journal that verifies clean. Proving this record against root
would need a witness root cannot reach -- an append-only store elsewhere, or a
signing key held off the host -- and Switchyard has neither.
