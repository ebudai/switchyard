...
Exactly the 3 files in the diff stat, no test files reference `kMaxDustSurfels`/`kDustLeafCount`/`SpiralDustLaneStrength`/any of the old numeric literals — nothing at risk of a stale assumption elsewhere.
...
Traced every new per-frame code path against `cutSurfelCount_` (up to `kMaxCutSurfels = 65536`):
* `AccumulateCoverageTelemetry` — called once per surfel from `EmitSurfel` during `SelectCut`'s traversal, and again once per surfel from `RecomputeCoverageTelemetryFromCut`'s loop. Each call is **O(1)**: a handful of arithmetic ops, no search over other surfels.
* `ApplyCoverageFootprintFloor` — a single `for (i < cutSurfelCount_)` loop, O(1) work per surfel (no nested loop, no lookup structure).
* The "spacing estimate" (`estimatedScreenSpacingPx_ = sqrt((width·height)/cutSurfelCount_)`) is **not** a nearest-neighbor search at all — it's a closed-form analytic proxy (bounding-box screen area of the whole cut, divided by count, square-rooted), computed once in `FinalizeCoverageTelemetry` from the four running min/max accumulators. This is exactly what the commit message calls it: a "screen-space spacing proxy," not literal per-splat nearest-neighbor distances — a deliberate O(1) approximation, not an accidentally-quadratic one.

Total added per-frame cost: **3 linear passes** over `cutSurfelCount_` (once during the existing `SelectCut` recursion — no new pass, since `AccumulateCoverageTelemetry` piggybacks on `EmitSurfel`'s call site — plus the floor pass plus the recompute pass), each strictly O(1) per surfel. **No O(n²) anywhere.** At 65536 surfels this is negligible next to the existing per-frame GPU upload/draw cost.
...
`git diff origin/main...HEAD --name-only` returns exactly `galaxy_system.h` and `shaders/dust_extinction.wgsl` — no `renderer.h`, no `main.cpp`, no `tuning_panel.h` (the three files the just-merged coverage-overlap/footprint-floor branch touched). Went further than a filename check: `git diff origin/main...HEAD -- galaxy_system.h | grep -E "^[+-]" | grep -iE "nebula|coverage|footprint|ApplyCoverageFootprintFloor|EmitSurfel|VisibleRadiusPx"` returns **nothing** — no added or removed line anywhere in this diff even mentions the star coverage-floor machinery or the nebula path by name. `find shaders -name "*.wgsl"` lists 4 shader files (`dust_extinction.wgsl`, `hardware_point.wgsl`, `postprocess.wgsl`, `splat.wgsl`); the diff touches only `dust_extinction.wgsl` — `splat.wgsl` (the star/coverage-floor consumer) and `postprocess.wgsl` (bloom/tonemap, also nebula's HDR feed) are untouched. The recently-merged smooth-haze fix is not perturbed by this branch.
...
`git diff origin/main...HEAD -- galaxy_system.h | grep -E "^[+-]" | grep -iE "kMaxDustSurfels|kDustLeafCount|kMaxNebulaSurfels|kNebulaLeafCount|kMaxCutSurfels"` returns nothing — this branch touches no buffer-sizing constants at all, so the buffer-sizing consistency check from the prior three passes doesn't apply here. Noting per the director's instruction rather than skipping silently.
...
Checked the strongest possible version of this: `StellarCatalogEntry` (`galaxy_model.h`) has exactly 6 fields — `temperatureMin/Max`, `luminosityMin/Max`, `oldWeight`, `youngWeight` — **there is no radius field in the catalog at all**, so there's no physical-radius value even present to accidentally wire into rendering. `surfelRadius` is computed by the same per-`ComponentKind` formula as before this branch (confirmed unchanged — the `switch(comp.kind)` block that sets `surfelRadius` in `GenerateStar` had only its `brightness = ...` lines removed in `663ac8b`; every `surfelRadius = ...` line is untouched), then threaded through as a plain parameter into `ApplyStellarPopulationColor`/`StarSample` without ever touching the catalog. Grepped the full branch diff for every line touching `surfelRadius`/`.radius` — confirmed all of them are pure pass-through plumbing (parameter passing, struct field assignment), none derive a new value from stellar type. Render footprint size is provably independent of stellar type in this implementation.
...
No test file references `StarSample`, `SurfelNode`, `colorWeight`, `BlackbodyToRGB`, or `kStellarCatalog` — nothing at risk of a stale assumption.

## 8. Verdict

**Clean.** The decouple — the property flagged as most important — is confirmed correct at the level the director asked for: `node.colorWeight = Σ(child.colorWeight)` with leaf `colorWeight = max(realLuminosity, 1e-4)`, node color computed as `Σ(colorWeight_i·color_i)/Σ(colorWeight_i)`, while the entire amplitude/energy path (`brightnessSum`, position weighting, covariance weighting, `node.brightness`, and everything downstream through `EmitSurfel`/the coverage floor) is untouched by even a single line in this branch — not merely equivalent in effect, literally absent from the diff. No NaN/div0 anywhere: every new denominator (`colorWeightSum`, the tonemap's luminance divisor, `aces_filmic_scalar`'s internal denominator) is either provably non-zero by construction or explicitly epsilon-floored, and `BlackbodyToRGB`'s temperature clamp makes every subsequent `log`/`pow` domain-safe unconditionally. The Radius Trap is structurally avoided — the catalog carries no radius field at all, and `surfelRadius` is confirmed to be pure unchanged pass-through plumbing. The blackbody formula matches the doc's own cited R formula literally and uses the fuller canonical coefficients (from the same standard reference family) for G/B; catalog weights match the doc's stated numbers where given and use the doc's own explicit "tunable" latitude for the non-doc-specified IMF-shaped young weights. The blue-saturation polish is correctly hot-star-gated (not global), bounded to the stated max 3x, and applied once at leaf generation only. Build is clean. Ready to merge pending Eric's sign-off on the blue saturation level, as the director noted.
