...
Installs the main-branch guard in every bare repository under /data/git and
installs complete repository policy for registered Switchyard projects plus
the agent and Eric PGU working checkouts. Requires root for the production
paths. Environment overrides exist for isolated tests.
EOF
}
