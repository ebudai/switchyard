...
readonly INSPECTOR_TARGET_PATTERN="pgu-inspector*"
...
REPO_ROOT="${PGU_INSPECTOR_GIT_GUARD_REPO_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}"
GLOBAL_HOOKS_DIR="${PGU_GLOBAL_GIT_HOOKS_DIR:-$HOME/.config/pgu/git-hooks}"
...
pre-commit and pre-push when PGU_PANE_TARGET matches ${INSPECTOR_TARGET_PATTERN},
...
write_pre_commit_hook() {
    ...
    local warning_helper="$GLOBAL_HOOKS_DIR/pgu-warn-file-size-limit.py"
...
if [[ "\${!ALLOW_ENV_NAME:-}" == "\$ALLOW_ENV_VALUE" ]]; then
    exit 0
fi
