...
REPO_ROOT="${UPDATE_HOOK_REPO_ROOT:-${PGU_UPDATE_HOOK_REPO_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}}"
LOCAL_REPO_ROOT="${UPDATE_HOOK_LOCAL_REPO_ROOT:-${PGU_UPDATE_HOOK_LOCAL_REPO_ROOT:-$REPO_ROOT}}"
SERVER_HOOKS_DIR="${UPDATE_HOOK_SERVER_HOOKS_DIR:-${PGU_UPDATE_HOOK_SERVER_HOOKS_DIR:-/data/git/pgu.git/hooks}}"
...
if [[ "\$refname" == "refs/heads/main" && "\${!ALLOW_ENV_NAME:-}" != "\$ALLOW_ENV_VALUE" && "\${!LEGACY_ALLOW_ENV_NAME:-}" != "\$ALLOW_ENV_VALUE" ]]; then
    cat >&2 <<MSG
