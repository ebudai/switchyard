...
- `commit_hash` stores the verified git commit associated with the ticket when it moves through `submit_to_audit` or `mark_done`; it is not writable through generic `edit_fields`, and director hash correction should re-run `mark_done` on the ticket so the correction uses the audited close verb
