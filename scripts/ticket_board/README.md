...
- `commit_hash` stores the verified git commit associated with the ticket when it moves through `submit_to_audit` or `mark_done`; the app verifies non-empty hashes against `TICKET_BOARD_COMMIT_GIT_DIR` before calling the database action, where that environment value may be one git repository path or a colon-separated ordered list of candidate repositories. New writes store the resolved full commit hash, and a write fails with a clear repository-path error if every configured verification repository is missing. The SQL schema still enforces hex shape, empty hashes remain absent rather than verified, existing stored hashes are not rewritten, and director hash correction should re-run `mark_done` on the ticket so the correction uses the audited close verb
...
- When a ticket is legitimately waiting on another pane, use `scripts/ticket-board-write await-role PGU-N --role perf|inspector|audit|...`; active awaiting markers suppress stall nudges, clear when that role ACTS on the ticket -- a state change or a reassignment -- and expire after the fallback timeout. Do not await your own role: the guarded write path rejects it, and a direct same-actor marker would be self-defeating because the same ticket-activity rule clears it. A comment does not clear it, including a comment by the awaited role: an acknowledgement is not an action, and treating it as one silently dropped escalations (PGU-909/PGU-915). An awaited role whose deliverable is the comment itself clears the marker with `clear-awaiting-role`
