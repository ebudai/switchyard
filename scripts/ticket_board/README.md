...
- Notification delivery is traced append-only in `ticket_board.notification_trace`; use `scripts/ticket_board/dump_notification_trace.py --ticket-id PGU-N` to inspect enqueue/claim/defer/send/drop/ack history for spurious-ping reports.
