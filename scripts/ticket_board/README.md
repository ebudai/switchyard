...
- When a ticket is legitimately waiting on another pane, use `scripts/ticket-board-write await-role PGU-N --role perf|inspector|audit|...`; active awaiting markers suppress stall nudges, clear when that role ACTS on the ticket -- a state change or a reassignment -- and expire after the fallback timeout. A comment does not clear it, including a comment by the awaited role: an acknowledgement is not an action, and treating it as one silently dropped escalations (PGU-909). An awaited role whose deliverable is the comment itself clears the marker with `clear-awaiting-role`
