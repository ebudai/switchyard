...
SCRIPT_CORE = """    const TICKET_REF_PATTERN = /\\b(?:[a-z0-9_]+:)?([A-Z][A-Z0-9]*-\\d+)\\b/ig;
    ...
    const createAttachImageBtn = document.getElementById('createAttachImageBtn');
    ...
    // notice was dead-lettered (SYRD-264), and a notice can be sent and never
    // seen to arrive (SYRD-268). The highlight stays the card's only cue --
    // the User found a status line beside it redundant (SYRD-266) -- so why a
    // notice has not arrived is on the card's hover and in the ticket view.
    // Empty when the notice was delivered: nothing is claimed either way.
    function activeWorkDeliveryHint(ticket) {
      ...
      if (delivery.state === 'failed') {
        summary = `Not delivered to ${owner}: ${delivery.reason || 'failed'}`;
      } else if (delivery.state === 'pending') {
        const tries = delivery.attempts ? ` (attempt ${delivery.attempts})` : '';
        summary = `Not yet delivered to ${owner}${tries}`;
      } else if (delivery.state === 'unconfirmed') {
        summary = `Sent to ${owner}, not confirmed received`;
      } else {
        summary = `No notice recorded for ${owner}`;
      }
      const reason = delivery.state === 'failed' ? '' : delivery.reason;
      return [summary, reason, delivery.at, delivery.next_attempt_at && `next try ${delivery.next_attempt_at}`]
        .filter(Boolean).join(' · ');
    }
