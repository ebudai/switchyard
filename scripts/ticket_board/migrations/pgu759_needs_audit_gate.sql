...
ALTER TABLE ticket_board.tickets
    ADD COLUMN IF NOT EXISTS needs_audit boolean;

UPDATE ticket_board.tickets
SET needs_audit = true
WHERE needs_audit IS NULL;

ALTER TABLE ticket_board.tickets
    ALTER COLUMN needs_audit SET DEFAULT true,
    ALTER COLUMN needs_audit SET NOT NULL;

UPDATE ticket_board.tickets
SET source_json = source_json || jsonb_build_object('needs_audit', needs_audit)
WHERE NOT source_json ? 'needs_audit';

UPDATE ticket_board.workflow_stages
SET entry_gate_field = 'needs_audit',
    gate_skip_to = 'director_review',
    exit_signoff_field = 'audit_signoff'
WHERE name = 'audit';
...
VALUES ('pgu759_needs_audit_gate.sql')
