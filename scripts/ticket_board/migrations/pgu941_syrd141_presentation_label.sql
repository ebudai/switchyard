-- SYRD-141: the presentation label a role's pane carries.
--
-- One optional role field, checked exactly as hard as every other string this
-- schema hands to a terminal: present means a non-empty single line. Absent
-- means the label provisioning computes -- an implementer reads
-- `<role> Developer`, anything else reads its own name -- which is decided in
-- the projection rather than here, because it is a presentation default and
-- not a rule about what a valid document is.
--
-- Nothing is backfilled. A tenant that names no label keeps the behaviour it
-- had, which is what a default means.
