...
-- neither heard anything. The cause was not the staged-pointer directorctl
-- delivery the incident went through: returning a ticket enqueues a
-- `transition` notification to the DIRECTOR as well as to the new owner, and
-- this guard skipped any ticket for which the Director had ANYTHING queued. The
-- Director's own kickback armed a suppression that lasted until that row was
-- delivered, and because both notifications were enqueued in one loop it took
-- the implementer's repair prompt with it.
--
-- Recovery is now staged: a turn ending unresolved prompts the OWNER, and the
-- Director is told only if the ticket is still unresolved after a grace period
-- or the prompt could not be delivered. The escalation half runs on ordinary
-- passes rather than turn ends, because a silent owner ends no turns and that
-- is precisely the case it exists for.
--
-- Numbered after pgu949 deliberately: ticket-board-migrate applies these in
-- alphabetical order, so a lower number would run BEFORE
-- pgu948_syrd194_unresolved_turn_guard.sql and be overwritten by it on any
-- clean build or replay (SYRD-203 review).
BEGIN;
...
-- The drop above takes the old grant with it. rbac.sql grants the new signature
-- on a rebuilt board; a board that arrives here by migration needs it restated,
-- or the listener cannot call the guard at all.
