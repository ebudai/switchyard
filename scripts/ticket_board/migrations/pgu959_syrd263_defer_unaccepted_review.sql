-- SYRD-263: the Director could not defer an unaccepted User Review ticket.
--
-- The declared executor requires a stage's sign-off before any transition
-- leaves it, except `return` and `reopen`. `defer` is a `move` to Backlog, so
-- from a review nobody has signed yet -- User Review with user_signoff false,
-- an inspection or audit not yet judged -- it was refused with
-- "stage signoff required". The Director was left to decide the review in
-- order to schedule it, or to reach for manual control.
--
-- Parking is not a verdict. A parking stage is not terminal, owns nobody and
-- notifies nobody, so nothing is promoted by landing there, and the move
-- grants no sign-off and clears none: the audited commit and every review
-- flag ride along unchanged. The exemption is keyed on the destination's
-- shape through `declared_parking_stage`, exactly as the blocker exemption of
-- SYRD-192 is, so it holds for a tenant that names its parking stage
-- something else. Every other transition out of a review still requires the
-- review's sign-off.
--
-- This redefines `enforce_declared_ticket_update` whole, with a body identical
-- to schema.sql's; the drift guard compares the two. Idempotent: CREATE OR
-- REPLACE only.
BEGIN;
