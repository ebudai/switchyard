...
def model_catalog(
    ...
    *,
    ...
    #: Why the live list could not be read, in the caller's own words, when the
    #: caller knows something this module cannot. `switchyard new` chooses
    #: models BEFORE it creates the owner account, and "run this on a host that
    #: can reach the runtime" is a wrong and unhelpful thing to tell somebody
    #: whose host is fine and whose account does not exist yet (SYRD-250 DAT).
    ...
    if unverified_because:
        detail = unverified_because
    else:
