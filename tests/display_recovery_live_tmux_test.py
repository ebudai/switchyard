...
def test_with_no_window_at_all_an_attached_recovery_succeeds(h: Harness) -> None:
    """Headless: nothing is on any screen, so visibility is not the test.

    The rule is "if a window is showing this presentation, the recovered
    display must be on it" -- not "refuse wherever nobody is looking", which
    would make `present recover` unusable on a tenant with no desktop.
    """
    config, config_path = h.config()
    state_path = h.root / "presentation.json"
    h.close()
    for slot in OPENED:
        wait_for(lambda s=slot: not clients_of(display(s)), f"the window to leave {display(slot)}")
    tmux("detach-client", "-t", pane(0, "#{pane_tty}"), check_rc=False)
    wait_for(lambda: not nested_attached(0, "director"), "the nested attach to close")
    before = {slot: pane(slot, "#{pane_pid}") for slot in (1, 2, 3)}

    state = recover(config, config_path, state_path)
    check(nested_attached(0, "director"), "the display is attached to its worker")
    check(state["history"][-1]["detail"]["slots"] == [0], "and the recovery is recorded")
    report = pc.presentation_report(config, config_path=config_path, state_path=state_path,
                                    runner=subprocess.run)
    check(report["window_attached"] is False, f"while the report says no window is attached: {report['window_attached']}")
    for slot in (1, 2, 3):
        check(pane(slot, "#{pane_pid}") == before[slot], f"slot {slot} untouched")
    h.open_window()


def test_attached_but_on_no_screen_is_not_a_recovery(h: Harness) -> None:
    """mefp exactly: the Director's WINDOW pane had itself exited.

    Its display slot proxied perfectly to the worker and was on no screen at
    all, so "attached" must not be reported as "recovered". The refusal names
    the command that window pane was running, and that command -- run for real
    here, as a new pane would -- is what brings it back.
    """
    config, config_path = h.config()
    state_path = h.root / "presentation.json"
    history_before = list(pc._read_state(state_path, config=config, config_path=config_path)["history"])
    before = {slot: pane(slot, "#{pane_pid}") for slot in (1, 2, 3)}

    # The window pane showing display 0 closes, and so does the nested attach.
    window_pane = h.outer.pop(0)
    window_pane.kill()
    window_pane.wait()
    wait_for(lambda: not clients_of(display(0)), "the window pane on display 0 to go")
    tmux("detach-client", "-t", pane(0, "#{pane_tty}"), check_rc=False)
    wait_for(lambda: not nested_attached(0, "director"), "the nested attach to close")
    # Presentation's own viewer still has a pane on display 0 -- a real client,
    # and one nobody can see, because no window is on the viewer. It must not
    # count as the display being shown.
    viewer = tl.viewer_session_for_project(PROJECT)
    tmux("new-session", "-d", "-x", "80", "-y", "24", "-s", viewer,
         f"env TMUX= tmux attach -t ={display(0)}")
    viewer_tty = tmux("display-message", "-p", "-t", f"={viewer}:0.0", "#{pane_tty}")
    wait_for(lambda: viewer_tty in clients_of(display(0)), "the viewer pane to attach to display 0")
    ...
    try:
        ...
        raise AssertionError("a display on no screen was reported as recovered")
    check("attached to its worker again in display slot(s) 0" in message, f"what did happen: {message}")
    check("no window is showing it" in message, f"what did not: {message}")
    check("not reported as a recovery" in message, f"and so what is not claimed: {message}")
    check(nested_attached(0, "director"), "the proxy itself is reattached")
    check(pc._read_state(state_path, config=config, config_path=config_path)["history"] == history_before,
          "no recovery is recorded")
    for slot in (1, 2, 3):
        check(pane(slot, "#{pane_pid}") == before[slot], f"slot {slot} untouched")

    # The printed remedy, run exactly as printed, as a new window pane.
    remedy = message.rsplit("run this in a terminal on the desktop: ", 1)[1].strip()
    check(remedy == f"env TMUX= tmux attach -t ={display(0)}",
          f"it is the command a window pane for slot 0 runs: {remedy!r}")
    h.outer.insert(0, subprocess.Popen(
        ["script", "-qfc", remedy, "/dev/null"],
        stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        env={**os.environ, "TERM": "xterm-256color"},
    ))
    wait_for(lambda: clients_of(display(0)), "the remedy to put a window on display 0")

    state = recover(config, config_path, state_path)
    check(state["history"][-1]["detail"]["slots"] == [0], "and now it is a recovery, recorded")
    for slot in (1, 2, 3):
        check(pane(slot, "#{pane_pid}") == before[slot], f"slot {slot} still untouched")
...
def main() -> int:
    ...
    try:
        for case in (
            ...
            test_with_no_window_at_all_an_attached_recovery_succeeds,
            test_attached_but_on_no_screen_is_not_a_recovery,
