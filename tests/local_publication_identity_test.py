...
def upgrade_with_remote(config_path: Path, remote: str, *, dry_run: bool = False, result=None):
    ...
    try:
        with identity_reported_as(False) as asked:
            ...
            code, output, _migrations = identity.cutover._upgrade(
...
def test_a_remote_that_is_not_one_stops_the_upgrade_before_anything_changes() -> None:
    """Fatal and first: not a warning after the phases with a zero exit."""
    for dry_run in (False, True):
        with tempfile.TemporaryDirectory(prefix="local-identity-pin-bad.") as tmp:
            ...
            config_path, _plan = tenant_with_owner_home(tmp_path, home)
            codes: list[int] = []
            output, runner, _asked = upgrade_with_remote(
                config_path, "/data/git/fix patch\nevil", dry_run=dry_run, result=codes
            )
            assert codes == [1], (dry_run, codes, output)
            assert "refusing to upgrade porter" in output and "Nothing was changed" in output, output
            assert not _registration().exists()
            assert runner.calls == [], ("the upgrade ran commands after a failed pin", runner.calls)


def test_a_pin_that_cannot_be_written_stops_the_upgrade() -> None:
    with tempfile.TemporaryDirectory(prefix="local-identity-pin-blocked.") as tmp:
        tmp_path = Path(tmp)
        home = owner_home_with(tmp_path, keys=(DEFAULT_KEY,))
        config_path, _plan = tenant_with_owner_home(tmp_path, home)
        _registration().mkdir(parents=True)  # something that is not a file, in its place
        codes: list[int] = []
        output, runner, _asked = upgrade_with_remote(config_path, LOCAL_REMOTE, result=codes)
        assert codes == [1], (codes, output)
        assert "is not a regular file" in output and "Nothing was changed" in output, output
        assert runner.calls == [], runner.calls


def test_the_cleanup_never_follows_the_tenant_plan_to_decide() -> None:
    """A privileged command must not read through a tenant symlink (SYRD-229 review).

    The tenant plan is replaced with a link to a file claiming the remote's host
    as its GitHub alias. Were it followed, the cleanup would take the tenant for
    a GitHub one; it must ignore it and refuse the link when it reads the plan.
    """
    with tempfile.TemporaryDirectory(prefix="identity-clear-link.") as tmp:
        home, config_path, tenant_plan = _stray(Path(tmp))
        _pin_remote("git@forge.invalid:owner/repo.git")
        decoy = Path(tmp) / "decoy.json"
        data = json.loads(tenant_plan.read_text(encoding="utf-8"))
        data["owner_github_host_alias"] = "forge.invalid"
        decoy.write_text(json.dumps(data), encoding="utf-8")
        tenant_plan.unlink()
        tenant_plan.symlink_to(decoy)
        with trusted_owner(home) as root_plan:
            _root_plan_with_identity(root_plan)
            # Root's own plan records no alias, so the linked tenant plan is the
            # only place one could come from -- which is what this case is about.
            root_data = json.loads(root_plan.read_text(encoding="utf-8"))
            root_data["owner_github_host_alias"] = ""
            root_plan.write_text(json.dumps(root_data), encoding="utf-8")
            result, output, runner = clear(config_path, dry_run=True)
        assert result == 1, output
        assert "is GitHub" not in output, "the tenant's linked plan decided the verdict"
        assert "publishes to git@forge.invalid:owner/repo.git, not GitHub" in output, output
        assert decoy.read_text(encoding="utf-8") == json.dumps(data)
