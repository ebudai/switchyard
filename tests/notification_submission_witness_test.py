...
def unconfirmed_because(conn, reason: str, witnessed) -> None:
    """Sent, taken off the queue, not retyped -- and never recorded delivered."""
    check("send" not in events(conn) and "listener_ack" not in events(conn),
          f"no send or listener_ack is recorded ({reason}): {events(conn)}")
    check(nl.SEND_UNCONFIRMED_EVENT in events(conn), f"it is recorded unconfirmed ({reason}): {events(conn)}")
    detail = detail_of(conn, nl.SEND_UNCONFIRMED_EVENT)
    check(detail["decision_reason"] == reason, f"with why: {detail}")
    check(detail["submission"]["witnessed"] is witnessed, f"and the witness's own answer: {detail['submission']}")
    unconfirmed_trace = next(trace for trace in conn.traces if trace and trace[4] == nl.SEND_UNCONFIRMED_EVENT)
    check(reason in unconfirmed_trace, f"the trace's reason column carries it for the board: {unconfirmed_trace}")
    check(conn.acked == [57] and conn.requeued == [] and conn.dead_lettered == [],
          f"taken off the queue, not retyped: {conn.acked} {conn.requeued} {conn.dead_lettered}")


def test_a_hook_state_that_is_gone_after_the_send_is_not_delivery() -> None:
    """The REAL gate saw the Director idle, and then there was nothing to read.

    The recipient's state file went away during the send -- its session was
    restarted and its state cleared. "Cannot tell" is not receipt (SYRD-268
    Final Sign-Off): the notice is unconfirmed, for its own reason.
    """
    def state_cleared(store, target):
        store._target_path(target).unlink()

    delivered, conn, sent = deliver(state_cleared)
    check(sent == [DIRECTOR] and delivered == 0, f"sent, and not counted delivered: {sent} {delivered}")
    unconfirmed_because(conn, nl.NO_HOOK_STATE, None)


def test_a_gate_that_cannot_be_asked_is_not_delivery() -> None:
    """An activity gate with no witness at all: unknown, so unconfirmed."""
    delivered, conn, _sent = deliver(None, gate="fake")
    check(delivered == 0, "not counted delivered where nothing can be asked")
    unconfirmed_because(conn, nl.NO_SUBMISSION_WITNESS, None)


def test_a_turn_nobody_started_says_so() -> None:
    delivered, conn, _sent = deliver(None)
    check(delivered == 0, "nothing witnessed")
    unconfirmed_because(conn, nl.NO_SUBMISSION_WITNESSED, False)
