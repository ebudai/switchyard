...
def test_the_whole_operator_sequence_runs_including_its_last_command() -> None:
    ...
    # And `--publish-remote` is remembered by root, end to end: the real
    # `sudo switchyard upgrade ... --publish-remote` recorded it, and the real
    # `sudo switchyard set-owner-identity porter --clear --dry-run` found it
    # (SYRD-229, the live mefp refusal).
    assert report["publish_remote_pinned"] == "git@example.invalid:owner/repo.git", report
    assert report["publish_remote_pin_uid"] == 0, report
    cleared = report["clear_dry_run_text"]
    assert "not established" not in cleared, cleared
    assert "publishes to git@example.invalid:owner/repo.git, not GitHub" in cleared, cleared
    # And a pin that cannot be kept stops the real command before anything
    # changes: non-zero, no success claimed, root's provision directory, the
    # tenant and the boundary state exactly as they were (SYRD-229 review).
    for label, failure in report["pin_failures"].items():
        assert failure["exit"] != 0, (label, failure)
        assert "refusing to upgrade porter" in failure["text"], (label, failure["text"])
        assert "Nothing was changed" in failure["text"], (label, failure["text"])
        assert failure["changed_nothing"], (label, "the failed pin left upgrade changes behind")
        for claim in ("refreshed porter", "recorded porter's publication remote", "recorded the way back"):
            assert claim not in failure["text"], (label, claim, failure["text"])
