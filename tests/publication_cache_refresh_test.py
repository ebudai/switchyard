    ...
    with temporary_cluster(prefix="syrd125-cache-", shutdown="immediate") as cluster:
        ...
        with tempfile.TemporaryDirectory(prefix="syrd125-") as tmpdir:
            ...
            try:
                ...
                # A BOARD OLDER THAN THIS PROGRAM cannot say where it verifies,
                # and one that old has no refresh-on-miss either -- so the
                # commit really may not be submittable. Said out loud rather
                # than skipped silently, because silence is what made SYRD-122
                # look like a working path.
                t.seed_postgres_ticket(admin, "PGU-4", title="Older board",
                                       state="in_progress", assignee="main")
                git("checkout", "-qb", "feature/pgu-4", cwd=worktree)
                fourth = commit_in(worktree, "pgu-4-change")
                original_config = t.TicketBoardServer.__mro__  # keep a reference for clarity
                del original_config
                import scripts.ticket_board.server as server_module

                real_handler = server_module.TicketBoardHandler.do_GET

                def older_board(self):  # noqa: ANN001
                    if self.path == "/api/client-config":
                        self.send_json({"build_id": "old", "write_token": self.write_token,
                                        "project": PROJECT, "project_name": "Cerulean",
                                        "ticket_prefix": "PGU"})
                        return
                    return real_handler(self)

                server_module.TicketBoardHandler.do_GET = older_board
                try:
                    aged = publish(worktree, "PGU-4", board_url=base)
                finally:
                    server_module.TicketBoardHandler.do_GET = real_handler
                assert aged.returncode != 0, aged.stdout
                text = aged.stdout + aged.stderr
                assert "does not say where it verifies commits" in text, text
                assert "IS published" in text, text
                # The push still landed, and the ticket did not move.
                assert git("--git-dir", str(remote), "rev-parse",
                           "refs/heads/roles/main/pgu-4") == fourth
                assert app.get_ticket("PGU-4")["commit_hash"] == ""
                checks += 1
