...
SET document = jsonb_set(document, '{roles}', (
    SELECT jsonb_agg(jsonb_set(role, '{capabilities}',
        (SELECT coalesce(jsonb_agg(c), '[]'::jsonb)
         ...
         WHERE c NOT IN ('request_publication', 'resolve_publication', 'recover_stalled_ticket'))) ORDER BY ordinality)
...
SET definition = jsonb_set(definition, '{capabilities}',
    (SELECT coalesce(jsonb_agg(c), '[]'::jsonb)
     ...
     WHERE c NOT IN ('request_publication', 'resolve_publication', 'recover_stalled_ticket')));
"""
