...
The journal is root's instead: root-owned directory, completed files 0444, a
chained index so a removed or edited attempt is visible rather than silent, and
a read path every role has without sudo.
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="syrd128-") as raw:
        ...
        # 7. THE CHAIN. It verifies as written, and a removed attempt is
        #    visible: an edited journal is the case root-ownership alone cannot
        #    answer, which is why the index is chained at all.
