...
def test_visible_layout_command_stays_owner_wrapped_without_forcing_pane_state_dir() -> None:
    with tempfile.TemporaryDirectory(prefix="pgu-team-launcher-visible-owner-command.") as tmp:
        ...
        # split's title. SYRD-139: two names, not one -- the window is the
        # project's and the split is the role's.
        assert argv[1:5] == ["--window-title", "porter", "--title", "Ops"], argv
        assert argv[5:] == team_launcher.pane_command_args(
            "porter",
            role,
            config_path=config_path,
            mode="attach-or-start",
