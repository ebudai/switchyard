...
class RealDesktopWindow:
    ...
    def __init__(self, session: str, *, env: dict[str, str]) -> None:
        ...
        self.env = env
        # The server this window belongs to, named rather than left to an
        # inherited `TMUX`. Taken from the environment the caller is already
        # driving that server with, so the client this opens and the sessions
        # the test made are the same server by construction (SYRD-219).
        tmpdir = env.get("TMUX_TMPDIR")
        if not tmpdir:
            raise AssertionError(
                "RealDesktopWindow was given an environment with no TMUX_TMPDIR, "
                "so it cannot tell which tmux server it is opening a client on"
            )
        self.socket = private_tmux_socket(Path(tmpdir))
        assert_private_tmux_socket(self.socket, Path(tmpdir))
        self.calls: list[dict[str, object]] = []
