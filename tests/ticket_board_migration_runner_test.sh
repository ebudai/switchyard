...
expected_transition_applied="$(
    {
        printf 'schema.sql\n'
        find "$TRANSITION_MIGRATIONS_DIR" -maxdepth 1 -type f -name '*.sql' -printf '%f\n' | sort
    } | paste -sd ','
)"
[[ "$transition_applied" == "$expected_transition_applied" ]] || {
    echo "FAIL: migration runner did not record the schema snapshot plus every migration filename" >&2
    echo "expected: $expected_transition_applied" >&2
    echo "actual:   $transition_applied" >&2
    exit 1
}

psql -X -v ON_ERROR_STOP=1 "$TRANSITION_CONN" <<'SQL' >/dev/null
UPDATE ticket_board.workflow_stages
SET owner_roles = ARRAY['builder']::text[]
WHERE name = 'in_progress';
SQL

pgu599_implementer_owner_result="$(
    psql -X -v ON_ERROR_STOP=1 -tA "$TRANSITION_CONN" <<'SQL'
SELECT jsonb_build_object(
    'builder', ticket_board.ticket_is_implementer_assignee('builder'),
    'ops', ticket_board.ticket_is_implementer_assignee('ops')
)::text;
SQL
)"
...
psql -X -v ON_ERROR_STOP=1 "$TRANSITION_CONN" <<'SQL' >/dev/null
UPDATE ticket_board.workflow_stages
SET owner_roles = ARRAY['main', 'app', 'ops', 'perf', 'research']::text[]
WHERE name = 'in_progress';
SQL

expected_pgu599_implementer_owner_result='{"ops": false, "builder": true}'
[[ "$pgu599_implementer_owner_result" == "$expected_pgu599_implementer_owner_result" ]] || {
    echo "FAIL: migrated DB ticket_is_implementer_assignee did not derive implementers from workflow_stages.owner_roles: $pgu599_implementer_owner_result" >&2
    exit 1
}
