...
def test_hook_state_writer_and_gate_idle_before_arrival_delivers_immediately() -> None:
    ...
    assert delivered == 1
    assert elapsed < 1.0
    assert sent == [_delivered("pgu-ops:0.0", "New ticket for you: PGU-304 -- Queue")]
...
def test_hermes_turn_end_idle_delivers_without_foreign_runtime_trace() -> None:
    ...
    assert sent == [_delivered("pgu-inspector:0.0", "PGU-774 -- Hermes notification")]
    assert conn.requeued == []
...
def test_real_idle_cursor_position_delivers_instead_of_wedging() -> None:
    ...
    assert sent == [_delivered("pgu-ops:0.0", "PGU-365 -- Ops notification")]
    assert conn.requeued == []
...
def test_cursor_home_delivers_regardless_of_row() -> None:
    for cursor_value in ("2 40 43", "2 42 43"):
        ...
        assert sent == [_delivered("pgu-ops:0.0", "PGU-365A -- Ops notification")]
...
def test_pre_send_recheck_delivers_when_cursor_stays_home() -> None:
    ...
    assert sent == [_delivered("pgu-ops:0.0", "PGU-364A -- Ops notification")]
    assert sleep_calls == [0.5]
    assert conn.requeued == []
...
def test_idle_role_resets_busy_backoff_before_claiming_due_notifications() -> None:
    ...
    assert sent == [_delivered("pgu-ops:0.0", "New ticket for you: PGU-627 -- Queue")]
    assert conn.requeued == []
...
def test_stale_codex_busy_state_recovers_and_delivers() -> None:
    ...
    assert sent == [_delivered("pgu-ops:0.0", "New ticket for you: PGU-558 -- Queue")]
    assert conn.requeued == []
...
def test_two_in_progress_tickets_do_not_mutually_finish_current_deadlock() -> None:
    ...
    assert sent == [_delivered("pgu-ops:0.0", "New ticket for you: PGU-334 -- Queue")]
...
def test_finish_current_releases_next_oldest_after_oldest_leaves_in_progress() -> None:
    ...
    assert sent == [_delivered("pgu-ops:0.0", "New ticket for you: PGU-336 -- Queue")]
...
def test_three_in_progress_tickets_deliver_oldest_first_one_at_a_time() -> None:
    ...
    assert sent == [_delivered("pgu-ops:0.0", "New ticket for you: PGU-100 -- Queue")]
...
def test_director_client_activity_latches_no_clobber_until_busy_idle_cycle() -> None:
    ...
    assert sent == [_delivered("pgu-director:0.0", "PGU-306 -- Director notification")]
...
def test_director_restart_window_holds_stale_idle_until_busy_idle_cycle() -> None:
    ...
    assert sent == [_delivered("pgu-director:0.0", "PGU-355 -- Director notification")]
...
def test_director_restart_window_timeout_releases_flat_idle_without_busy_cycle() -> None:
    ...
    assert sent == [_delivered("pgu-director:0.0", "PGU-358 -- Director notification")]
...
def test_director_delivery_matrix_for_trusted_and_untrusted_claude_idle_sources() -> None:
    ...
    assert trusted_idle_sent == [_delivered("pgu-director:0.0", "PGU-360 -- Director notification")]
    assert trace_events(trusted_idle_conn) == ["listener_claim", "send", "listener_ack"]
    assert trusted_idle_conn.traces[1][6] == "hook_idle"

    assert trusted_static_interrupt_sent == [_delivered("pgu-director:0.0", "PGU-360 -- Director notification")]
    ...
    assert untrusted_idle_sent == [_delivered("pgu-director:0.0", "PGU-360 -- Director notification")]
...
def test_idle_nudge_enumeration_does_not_touch_director_composing_latch() -> None:
    ...
    assert sent == [_delivered("pgu-director:0.0", "PGU-359 -- Director notification")]
...
def test_long_idle_live_pane_still_delivers() -> None:
    ...
    assert sent == [_delivered("pgu-research:0.0", "New ticket for you: PGU-778 -- Queue")]
    assert conn.requeued == []
...
def test_busy_existing_pane_holds_then_delivers_when_idle() -> None:
    ...
    assert listener.listen_once(max_notifications=1) == 1
    assert sent == [_delivered("pgu-ops:0.0", "New ticket for you: PGU-776 -- Queue")]
...
def test_final_review_handoff_survives_busy_retry_and_manual_control() -> None:
    ...
    assert listener.listen_once(max_notifications=1) == 1
    assert sent == [_delivered("pgu-director:0.0", message)]
...
def test_final_review_handoff_ignores_only_scheduling_flags() -> None:
    ...
    for notification_id, ticket_row in (
        ...
        assert listener.listen_once(max_notifications=1) == 1
        assert sent == [_delivered("pgu-director:0.0", f"New ticket for you: {ticket_id} -- Queue")]
...
def test_terminal_transition_notification_is_delivered_to_prior_owner() -> None:
    ...
    assert listener.listen_once(max_notifications=1) == 1
    assert sent == [_delivered("pgu-ops:0.0", message)]
    assert conn.acked == [9]
