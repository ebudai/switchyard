...
FROM ticket_board.ticket_notification_queue WHERE ticket_id='{ticket}';"""))
        ...
        try:
            ...
            # The DELIVERED case is the whole reason the check reads the trace
            # and not only the queue: an acknowledged escalation leaves no row,
            # and reading rows alone let this generator repeat the reminder
            # path's news a moment after it was answered.
            check(fixture.psql(admin,
                  "SELECT count(*) FROM ticket_board.notification_trace WHERE ticket_id='PGU-202' "
                  "AND kind='unresolved_turn' AND event='enqueue';").strip() == "1",
                  "the escalation was enqueued exactly once in the whole history")
            check("unresolved_turn" not in kinds_for("PGU-202", "director"),
...
END $$;""")
            ...
            # 7b. AND THE SCHEMA ITSELF DROPS THE OLD SIGNATURE. schema.sql is
            #     applied over existing databases as well as fresh ones, and
            #     CREATE OR REPLACE would leave the two-argument function beside
            #     the new one -- after which every two-argument call is
            #     ambiguous and the guard cannot be invoked at all.
            overload_db = "unresolved_turn_overload"
            overload_admin = fixture.conninfo(cluster.socket_dir, cluster.port, overload_db)
            fixture.run(["createdb", "-h", str(cluster.socket_dir), "-p", str(cluster.port),
                         "-U", "postgres", overload_db])
            fixture.psql(overload_admin, """
CREATE SCHEMA IF NOT EXISTS ticket_board;
CREATE OR REPLACE FUNCTION ticket_board.notify_unresolved_turn_end(
    p_turn_by_role jsonb, p_now timestamptz) RETURNS integer LANGUAGE sql AS $old$ SELECT 0 $old$;""")
            fixture.psql(overload_admin, fixture.SCHEMA_PATH.read_text())
            definitions = fixture.psql(overload_admin, """
SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname='ticket_board' AND p.proname='notify_unresolved_turn_end';""").strip()
            check(definitions == "1",
                  f"applying the schema over the old function replaces it: {definitions}")

            # 8. THE UPGRADE, IN THE ORDER THE RUNNER APPLIES IT. Adding the
            #    grace interval does not REPLACE this function -- CREATE OR
            #    REPLACE matches on parameter types -- it overloads it, and with
            #    a default on the new parameter a two-argument call then matches
            #    both and Postgres refuses it as ambiguous. The migration has to
            #    drop the old signature, and it has to sort after the migration
            #    that last defined it, because ticket-board-migrate applies
            #    these alphabetically and a lower number would be overwritten on
            #    any clean build or replay.
            upgrade_db = "unresolved_turn_upgrade"
            ...
            fixture.psql(upgrade_admin, show("scripts/ticket_board/schema.sql"))
            fixture.psql(upgrade_admin, show("scripts/ticket_board/rbac.sql"))
            migrations = sorted(
                path for path in (ROOT / "scripts" / "ticket_board" / "migrations").glob("pgu9*.sql")
            )
            applied = [path.name for path in migrations]
            check(applied.index("pgu950_syrd203_staged_turn_recovery.sql")
                  > applied.index("pgu948_syrd194_unresolved_turn_guard.sql"),
                  f"this migration runs after the one that last defined the guard: {applied}")
            for path in migrations:
                try:
                    fixture.psql(upgrade_admin, path.read_text(encoding="utf-8"))
                except AssertionError as exc:
                    check("already exists" in str(exc) or "does not exist" in str(exc),
                          f"{path.name} applied cleanly: {str(exc)[:200]}")
            definitions = fixture.psql(upgrade_admin, """
SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname='ticket_board' AND p.proname='notify_unresolved_turn_end';""").strip()
            check(definitions == "1",
                  f"exactly one definition survives the replay, not an overload: {definitions}")
            answered = fixture.psql(upgrade_admin, """
SET ROLE ticket_board_listener;
SELECT ticket_board.notify_unresolved_turn_end('{"app":"t"}'::jsonb, clock_timestamp());
RESET ROLE;""")
            check("not unique" not in answered,
                  f"and a two-argument call is not ambiguous: {answered[:120]}")
