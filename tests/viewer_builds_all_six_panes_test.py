...
This pane itself runs inside tmux. A bare `tmux` from here, including the
nested `tmux attach` every viewer pane runs, would reach the LIVE server, so
`TMUX` is removed and `TMUX_TMPDIR` is private for everything started here, and
the server's socket is checked to be the private one before anything is
created or killed.
"""
...
def private_tmux(binary: str | None = None):
    ...
    with tempfile.TemporaryDirectory(prefix="syrd221-viewer.") as tmp:
        ...
        socket = tmux("display", "-p", "#{socket_path}").stdout.strip()
        assert socket.startswith(str(tmp_path)), f"not a private server: {socket}"
        try:
            ...
            if tmux("display", "-p", "#{socket_path}").stdout.strip().startswith(str(tmp_path)):
                tmux("kill-server")
...
def _attach_a_window(env, rows: int, cols: int):
    ...
    # A terminal window always says what it is. Without TERM -- as under a
    # cleared test environment -- tmux cannot open the client at all, and a
    # viewer that "did not follow its window" would be measuring that.
    client_env = dict(env, TERM=env.get("TERM") or "xterm-256color")
    pid, fd = pty.fork()
    if pid == 0:  # pragma: no cover - replaced by exec
        os.execvpe("tmux", ["tmux", "attach", "-t", f"={PROJECT}-viewer"], client_env)
