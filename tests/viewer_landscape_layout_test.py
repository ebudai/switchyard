...
class Server:
    ...
    `TMUX` is stripped, and that is not belt and braces. A tmux command run
    with `TMUX` set and no `-L`/`-S` talks to the server that variable names
    and ignores `TMUX_TMPDIR` entirely -- so this suite, run from inside a tmux
    pane, would create its sessions on the caller's own server, set options on
    it, and end by killing it. That is not hypothetical: it is what happened to
    a live Switchyard desktop while this ticket was being written.

    The provenance check is therefore where the teardown is, not only at the
    start. Asking before any server exists proves nothing -- `display-message`
    simply fails, and an empty answer is not evidence of anything -- so
    `kill()` resolves the running server itself and refuses to issue
    `kill-server` unless that socket is inside this fixture's own directory.
    """

    def __init__(self) -> None:
        self.tmpdir = tempfile.mkdtemp(prefix="syrd216-tmux.")
        self.env = {k: v for k, v in os.environ.items() if k not in ("TMUX", "TMUX_PANE")}
        self.env["TMUX_TMPDIR"] = self.tmpdir
        # `tmux attach` refuses to start without a usable TERM, and inheriting
        # one is luck: run under a stripped environment the client never
        # attaches, the window keeps its creation size, and the resize cases
        # fail for a reason that has nothing to do with layouts.
    ...
    def socket_path(self) -> str | None:
        """The socket of the server this fixture's environment resolves to.

        None when there is no server yet, which is a different thing from a
        server that is not ours and must not be confused with one.
        """
    ...
    def verify(self) -> str:
        ...
        if not socket.startswith(self.tmpdir):
            raise AssertionError(
                f"refusing to continue: tmux resolved to {socket}, outside "
                f"{self.tmpdir}. Something is redirecting this fixture at "
                "another server, and the teardown here kills servers."
            )
        return socket

    def start(self, *args: str) -> subprocess.CompletedProcess[str]:
        """Create the first session, then prove what we just created is ours."""
        created = self.tmux(*args)
        self.verify()
        return created

    def tmux(self, *args: str, **kwargs: object) -> subprocess.CompletedProcess[str]:
        self.issued.append(["tmux", *args])
...
def test_the_arrangement_follows_the_window_through_a_real_resize() -> None:
    ...
    try:
        ...
        attached = "0"
        while time.monotonic() < deadline:
            attached = server.tmux("display-message", "-p", "-t", "v",
                                   "#{session_attached}").stdout.strip()
            if attached not in ("", "0"):
                break
            time.sleep(0.2)
        check(attached not in ("", "0"),
              f"no client attached to the viewer within 30s (session_attached={attached!r})")
...
def test_teardown_refuses_a_server_it_cannot_prove_is_its_own() -> None:
    """The guard has to bite, not merely be present.

    The first version of it asked `display-message` in `__init__`, before any
    server existed. tmux failed, the empty answer was read as "nothing
    unexpected", and the check passed for the one reason it should never pass.
    `kill()` then issued `kill-server` having proved nothing at all. Given what
    this code already did to a live desktop, that is worth a test of the
    refusal rather than of the happy path.
    """
    # 1. Nothing running. `verify()` must refuse rather than read an absent
    #    answer as reassurance -- that is exactly what the first version of
    #    this guard did, and why it passed while protecting nothing.
    quiet = Server()
    try:
        quiet.verify()
    ...
    #    Teardown of that same fixture kills nothing, and guesses nothing.
    quiet.kill()
    check(not any("kill-server" in args for args in quiet.issued),
          f"kill-server was issued with no server running: {quiet.issued}")

    # 2. A server that is somebody else's. Standing in for the live one, built
    #    in its own directory, and the fixture pointed at it.
    bystander = Server()
    try:
        bystander.start("new-session", "-d", "-s", "not-ours", "sleep 600")
        check(bystander.socket_path() is not None, "the bystander server did not start")

        misdirected = Server()
        misdirected.env["TMUX_TMPDIR"] = bystander.tmpdir
        resolved = misdirected.socket_path()
        check(resolved is not None and not resolved.startswith(misdirected.tmpdir),
              f"the misdirected fixture did not reach the other server: {resolved}")
        try:
            misdirected.kill()
        except AssertionError as refusal:
            check("refusing to kill" in str(refusal), f"unexpected refusal: {refusal}")
        else:
        ...
        # And the proof that matters: it is still alive.
    ...
    # 3. And `start` refuses too, rather than building on an unproved server.
    stray = Server()
    stray.env["TMUX_TMPDIR"] = "/nonexistent-for-syrd216"
    try:
        stray.verify()
    except AssertionError as refusal:
        check("refusing to continue" in str(refusal) or "no tmux server" in str(refusal),
              f"unexpected refusal: {refusal}")
    else:
        raise AssertionError("verify() accepted a server it could not place")
    shutil.rmtree(stray.tmpdir, ignore_errors=True)
