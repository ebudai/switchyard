...
class Server:
    ...
    a live Switchyard desktop while this ticket was being written, which is why
    `assert_private` below refuses to run at all unless the server really is
    this fixture's own.
    """
    ...
    def assert_private(self) -> None:
        """Prove the socket is ours before anything is created or killed."""
        socket = self.tmux("display-message", "-p", "#{socket_path}").stdout.strip()
        if socket and not socket.startswith(self.tmpdir):
            raise AssertionError(
                f"refusing to run: tmux resolved to {socket}, outside {self.tmpdir}. "
                "Something in the environment is redirecting this fixture at "
...
def test_a_portrait_window_stacks_instead_of_squeezing() -> None:
    ...
    try:
        for panes, shape in expected.items():
            session = f"p{panes}"
            server.tmux("new-session", "-d", "-x", str(TALL_WIDTH), "-y", str(TALL_HEIGHT),
                        "-s", session, "sleep 600")
            for _ in range(panes - 1):
                server.tmux("split-window", "-t", f"{session}:0", "sleep 600")
                server.tmux("select-layout", "-t", f"{session}:0", "tiled")
...
def test_more_columns_than_rows_is_not_the_same_as_wider_than_tall() -> None:
    ...
    server = Server()
    try:
        server.tmux("new-session", "-d", "-x", "120", "-y", "80", "-s", "n", "sleep 600")
        for _ in range(4):
...
def test_the_arrangement_follows_the_window_through_a_real_resize() -> None:
    ...
    try:
        ...
        for rows, columns, want in ((40, 200, [3, 2]), (100, 60, [2, 2, 1]), (40, 200, [3, 2])):
            fcntl.ioctl(master, termios.TIOCSWINSZ, struct.pack("HHHH", rows, columns, 0, 0))
            deadline = time.monotonic() + 15
            while time.monotonic() < deadline:
                got = rows_of(server.geometry("v"))
                if [len(row) for row in got] == want:
                    break
                time.sleep(0.25)
            got = rows_of(server.geometry("v"))
            check([len(row) for row in got] == want,
                  f"a {columns}x{rows} client left the viewer {[len(r) for r in got]}, not {want}")
